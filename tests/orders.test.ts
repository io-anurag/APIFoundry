...
describe("Orders resource", () => {
  ...
  describe("Authorization (orders:read / orders:write)", () => {
    it("returns 401 for the collection, single-record, and nested routes with no token", async () => {
      expect((await request(app).get(BASE)).status).toBe(401);
      expect((await request(app).get(`${BASE}/1`)).status).toBe(401);
      expect((await request(app).post(BASE).send({})).status).toBe(401);
      expect((await request(app).get(`${config.apiPrefix}/users/3/orders`)).status).toBe(401);
      expect((await request(app).get(`${BASE}/1/products`)).status).toBe(401);
    });

    it("returns 403 INSUFFICIENT_SCOPE for a token missing orders:read/orders:write", async () => {
    ...
    it("grants GET with only orders:read but rejects writes with 403", async () => {
      const readOnly = await issueScopedToken(["orders:read"]);
    ...
    it("grants writes with orders:write", async () => {
      const writer = await issueScopedToken(["orders:write"]);
  ...
  describe("Pagination, sorting, filtering (User Story 4)", () => {
    ...
    it("sorts ascending and descending by total (FR-009)", async () => {
      const asc = await request(app).get(`${BASE}?sort=total&limit=100`).set(...bearer(token));
    ...
    it("filters by status, alone and combined with pagination/sort (FR-010)", async () => {
      const res = await request(app).get(`${BASE}?status=pending&limit=100`).set(...bearer(token));
    ...
    it("resolves a repeated conflicting filter parameter deterministically", async () => {
      const first = await request(app).get(`${BASE}?status=pending&status=shipped`).set(...bearer(token));
      const second = await request(app).get(`${BASE}?status=pending&status=shipped`).set(...bearer(token));
  ...
  describe("Nested route: users/:id/orders (User Story 2)", () => {
    it("lists orders for a user linked via customer.userId", async () => {
      ...
      const res = await request(app).get(`${config.apiPrefix}/users/3/orders`).set(...bearer(token));
    ...
    it("returns 404 for a nonexistent user", async () => {
      const res = await request(app).get(`${config.apiPrefix}/users/999999/orders`).set(...bearer(token));
      expect(res.status).toBe(404);
    });
    ...
    it("returns 200 with empty data for a user with no linked customers/orders", async () => {
      ...
      const res = await request(app).get(`${config.apiPrefix}/users/2/orders`).set(...bearer(token));
      expect(res.status).toBe(200);
  ...
  describe("Nested route: orders/:id/products (User Story 2)", () => {
    it("lists the distinct products referenced by an existing order", async () => {
      const res = await request(app).get(`${BASE}/1/products`).set(...bearer(token));
      expect(res.status).toBe(200);
      expect(res.body.data.length).toBeGreaterThan(0);
    });

    it("returns 404 for a nonexistent order", async () => {
      const res = await request(app).get(`${BASE}/999999/products`).set(...bearer(token));
      expect(res.status).toBe(404);
    });
  });
