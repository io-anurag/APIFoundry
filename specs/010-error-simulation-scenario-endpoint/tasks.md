...
- [X] T001 [P] Create `src/data/testScenarios.catalog.ts` (data-model.md): `export const
      ...
      today").
- [X] T002 [P] Extract `renderStatusCodeDemo` in `src/services/statusCode.service.ts` (research.md
...
- [X] T003 [US1] Create `src/controllers/errors.controller.ts` (depends on T001, T002): `import
...
- [X] T004 [US1] Create `src/routes/errors.routes.ts` (depends on T003): `import { Router } from
...
- [X] T005 [US1] Wire `errorsRouter` into `src/app.ts` (depends on T004): `import { errorsRouter }
...
- [X] T006 [P] [US1] Create `tests/errorScenarios.test.ts` (depends on T005): for each of
...
- [X] T007 [P] [US2] Add `parseOptionalStatusCodeParam` to `src/utils/statusCodeParam.ts`
...
- [X] T008 [P] [US2] Create `src/models/testScenarioRequest.ts` (depends on T001, T007): `import {
...
- [X] T009 [US2] Create `src/services/testScenario.service.ts` (depends on T001): `import {
      ...
      behavior).
- [X] T010 [US2] Create `src/controllers/testScenario.controller.ts` (depends on T008, T009, T002):
...
- [X] T011 [US2] Create `src/routes/testScenario.routes.ts` (depends on T010): `import { Router }
...
- [X] T012 [US2] Wire `testScenarioRouter` into the existing versioned `apiRouter` in `src/app.ts`
...
- [X] T013 [P] [US2] Create `tests/testScenario.test.ts` (depends on T012): `GET
...
- [X] T014 [P] [US3] Extend `src/models/testScenarioRequest.ts` (depends on T008): add `import {
...
- [X] T015 [P] [US3] Extend `src/services/testScenario.service.ts` (depends on T009): add `import {
...
- [X] T016 [US3] Extend `src/controllers/testScenario.controller.ts` (depends on T014, T015): add
      ...
      res.status(status).json(payload); return; }` — deliberately **no** `scenario`/`status`
      wrapper here: FR-010 requires the response body's exact byte size to match Spec 007's
      "large" preset, and `generatePayload` already computes `data`'s filler length against a
      minimal `{size, data}` shell, so any additional field would inflate the body past that exact
      target (confirmed by an implementation-time test failure — the originally-planned wrapped
      shape measured 1,048,617 bytes against a 1,048,576-byte target). Otherwise (any of the ten remaining scenarios): `const
...
- [X] T017 [P] [US3] Extend `tests/testScenario.test.ts` (depends on T016): timing —
...
- [X] T018 [P] Merge `contracts/error-simulation.openapi.yaml`'s `tags`, `paths`,
...
- [X] T019 Run `npm test` and confirm the full suite — Specs 001-009's existing tests (including
...
- [X] T020 Execute the manual validation scenarios in
...
- [X] T021 [P] Spot-check `/docs` (Swagger UI), `/openapi.json`, and `/openapi.yaml` render the 10
