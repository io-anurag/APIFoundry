...
- [ ] T001 Add `@vitest/coverage-v8` as a devDependency (pin to the `^2.1.x` line to match the
...
- [ ] T002 [P] Create `k6/lib/config.js`: reads `BASE_URL` from k6's `__ENV` object, defaulting to
      ...
      ```
- [ ] T003 [P] Create `k6/lib/auth.js` (depends on T002): exports `login(username, password)` —
...
- [ ] T004 Run `npm install && npm test` on a clean checkout and confirm the pre-existing 42-file
      ...
      flag before proceeding, not something this feature's tasks should paper over.
...
- [ ] T005 [US1] Perform the CLAUDE.md Testing-Expectations checklist audit against the 42 existing
...
- [ ] T006 [US1] Add unsupported-method assertions to `tests/users.test.ts` (depends on T005):
...
- [ ] T007 [P] [US1] Add the same two unsupported-method assertions (collection `PATCH`, item `POST`)
      to `tests/products.test.ts` (depends on T005).
- [ ] T008 [P] [US1] Add the same two unsupported-method assertions to `tests/orders.test.ts`
      (depends on T005).
- [ ] T009 [P] [US1] Add the same two unsupported-method assertions to `tests/customers.test.ts`
      (depends on T005).
- [ ] T010 [US1] Run `npm run test:coverage` (depends on T001, T006-T009) and confirm statement
      ...
      run, since the exact number is not knowable before this task executes.
- [ ] T011 [US1] Run `npm test` twice in immediate succession (no manual reset step between runs)
      ...
      (most likely a test that depends on another test's leftover state, contra FR-004).
...
- [ ] T012 [US2] Create `k6/lib/workflow.js` (depends on T002, T003) exporting
...
- [ ] T013 [US2] Create `k6/chained-workflow.js` (depends on T012): `import { runChainedWorkflow }
...
- [ ] T014 [P] [US2] Create `k6/smoke.js` (depends on T002): `export const options = { vus: 1,
...
- [ ] T015 [P] [US2] Create `k6/load.js` (depends on T002): `export const options = { vus: 20,
      ...
      errors<1%).
- [ ] T016 [P] [US2] Create `k6/stress.js` (depends on T002): `export const options = { stages: [{
      ...
      p95<500ms, errors<1%).
- [ ] T017 [P] [US2] Create `k6/spike.js` (depends on T002): `export const options = { stages: [{
...
- [ ] T018 [P] [US2] Create `k6/soak.js` (depends on T002): `export const options = { vus: 20,
...
- [ ] T019 [P] [US2] Create `k6/latency.js` (depends on T002): no VU/duration threshold (functional,
...
- [ ] T020 [P] [US2] Create `k6/timeout.js` (depends on T002): calls
...
- [ ] T021 [P] [US2] Create `k6/error-rate.js` (depends on T002): no load-shaped VU/duration
...
- [ ] T022 [P] [US2] Create `k6/auth.js` (depends on T003): near-100%-pass functional script
...
- [ ] T023 [P] [US2] Create `k6/crud.js` (depends on T002): near-100%-pass functional script
...
- [ ] T024 [US2] Create `k6/concurrent-user.js` (depends on T012): `import { runChainedWorkflow }
...
- [ ] T025 [US2] Manually run every script in `k6/` against `npm run dev` (per quickstart.md
...
- [ ] T026 [US3] Edit `README.md`'s "Specs implemented so far" bullet list (currently ends at
...
- [ ] T027 [US3] Edit `README.md`: add a `013 — Automated Test Hardening, k6 Scenarios & README`
...
- [ ] T028 [US3] Edit `README.md`'s "## Run the tests" section (currently just `npm test`): add
      `npm run test:coverage` alongside it, one sentence noting it enforces the ≥85% statement-
      coverage gate (T001).
- [ ] T029 [US3] Add a new `## Examples` section to `README.md` (depends on T026-T028) with at least
...
- [ ] T030 [US3] Add a new `## Performance testing with k6` section to `README.md` (depends on
...
- [ ] T031 [US3] Follow `README.md` end-to-end from a clean checkout per quickstart.md Scenario 3 —
...
- [ ] T032 [P] Run `npm test` and confirm the full suite — every existing Specs 001-012 test file
      plus this feature's T006-T009 additions — passes.
- [ ] T033 Execute `specs/013-test-hardening-k6-readme/quickstart.md`'s "Final quality gate" section
      ...
      (FR-015, SC-006).
- [ ] T034 [P] Confirm `.gitignore`'s existing `coverage/` entry actually covers
      `@vitest/coverage-v8`'s output directory (it already does by name; this is a one-line
      double-check, not an edit, unless the tool writes elsewhere).
